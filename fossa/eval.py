...
def eval_models(X, y, models, n_splits=None, verbose=False):
    ...
    models : list of FossaPredictorABC based models. These models will be evaluated
    n_splits : integer
    ...
    for model in models:
        ...
        for category in categories:
            metrics[category] = {'TP': 0, 'FP': 0, 'TN': 0, 'FN': 0, 'num_samples': 0, 'num_values': 0}
        ...
        for train, test in tscv.split(datetimeindex):
            ...
            test_samples = X.loc[datetimeindex[test]]  # .reset_index().set_index(['date','category'])
            test_samples.index = test_samples.index.remove_unused_levels()

            test_labels = y.loc[datetimeindex[test]]  # .reset_index().set_index(['date','category'])
            test_labels.index = test_labels.index.remove_unused_levels()

            model.fit(train_samples, train_labels)
            prediction = model.predict(test_samples)

            results = pd.merge(prediction, test_labels, left_index=True, right_index=True)
            results = pd.merge(results, test_samples, left_index=True, right_index=True)
            ...
            for category in results.index.levels[1]:
                ...
                metrics[category]['TP'] = metrics[category]['TP'] + np.sum(
                    (category_results['direction'].values != 0) & (
                            category_results['direction'].values == category_results['is_anomaly'].values))
                metrics[category]['FP'] = metrics[category]['FP'] + np.sum(
                    (category_results['direction'].values != 0) & (category_results['is_anomaly'].values == 0))
                metrics[category]['TN'] += np.sum((category_results['direction'].values == 0) & (
                        category_results['is_anomaly'].values == 0))
                metrics[category]['FN'] += np.sum((category_results['direction'] == 0).values & (
                        category_results['is_anomaly'].values != 0))
                metrics[category]['num_samples'] += len(category_results)
                metrics[category]['num_values'] += np.sum(category_results['value'])
